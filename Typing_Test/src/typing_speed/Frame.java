...
public class Frame extends JFrame implements ActionListener, KeyListener
{
	...
	public Frame()
	{
		...
		label.setFont(new Font("Arial",Font.BOLD,30));
                                          label.setForeground(new Color(255, 3, 137));
		...
		this.getContentPane().setBackground(new Color(69, 66, 66));
	...
	public void draw(Graphics g)
	{
		...
		if(ended)
		{
			...
			else if(WPM>25 && WPM<=35)
				message="You are a Good Typist";
			else if(WPM>35 && WPM<=50)
				message="You are an Excellent Typist";
                                                               else if(WPM > 50 && WPM <= 75){
                                                                                message= "You are an Outstanding Typist";
                                                               }
                                                               else if(WPM > 75 && WPM <=100){
                                                                                message="You are an Elite Typist";
                                                               }
			else
				message="You are a Master Typist";
			...
			g.drawString("Typing Speed: "+WPM+" Words Per Minute" , (SCREEN_WIDTH-metrics.stringWidth("Typing Speed: "+WPM+" Words Per Minute"))/2, g.getFont().getSize()*8);
                                        g.drawString("Time: "+ end + "  seconds", (SCREEN_WIDTH-metrics.stringWidth("Time: "+ end + " seconds"))/2, g.getFont().getSize()*9);
                                        g.drawString("Letters: " + typed, (SCREEN_WIDTH-metrics.stringWidth("Letters: " + typed))/2, g.getFont().getSize()*10);
                                        
                                                                g.setColor(Color.RED);
			g.drawString(message, (SCREEN_WIDTH-metrics.stringWidth(message))/2, g.getFont().getSize()*12);
	...
	public static String getPassage()
	{
		...
		String pas1="Many touch typists also use keyboard shortcuts or hotkeys when typing on a computer. This allows them to edit their document without having to take their hands off the keyboard to use a mouse. An example of a keyboard shortcut is pressing the Ctrl key plus the S key to save a file";
		...
		String pas3="Frank Edward McGurrin, a court stenographer from Salt Lake City, Utah who taught typing classes, reportedly invented touch typing in 1888. On a standard keyboard for English speakers the home row keys are: \"ASDF\" for the left hand and \"JKL;\" for the right hand";
		...
		String pas6="Proofreader applicants are tested primarily on their spelling, speed, and skill in finding errors in the sample text. Toward that end, they may be given a list of ten or twenty classically difficult words and a proofreading test, both tightly timed. The proofreading test will 0";
		String pas7="In one study of average computer users, the average rate for transcription was 33 words per minute, and 19 words per minute for composition. In the same study, when the group was divided into \"fast\", \"moderate\" and \"slow\" groups, the average speeds were 40 wpm, 35 wpm, and 30 wpm";
		...
		String pas9="Web designers are expected to have an awareness of usability and if their role involves creating mark up then they are also expected to be up to date with web accessibility guidelines. The different areas of web design include web graphic design; interface design; ";
		String pas10="A data entry clerk is a member of staff employed to enter or update data into a computer system. Data is often entered into a computer from paper documents using a keyboard. The keyboards used can often have special keys and multiple colors to help in the task and speed up the process";
